...
class ExecutionEngine:
    ...
    def _resolve_value(self, val_obj):
        ...
        if isinstance(val_obj, dict) and val_obj.get("type") == "placeholder":
            if self.param_index >= len(self.current_params):
                raise ValueError("Not enough parameters provided for query placeholders.")
    ...
    def _execute_select(self, plan: Dict[str, Any]):
        ...
        # Filter
        if resolved_where:
            results = [row for row in results if self._evaluate_where(row, resolved_where)]
    ...
    def _execute_update(self, plan: Dict[str, Any]):
        ...
        # Resolve values
        resolved_assignments = {k: self._resolve_value(v) for k, v in assignments.items()}
        resolved_where = None
        if where:
            resolved_where = where.copy()
            resolved_where["value"] = self._resolve_value(where["value"])
    ...
    def _execute_delete(self, plan: Dict[str, Any]):
        ...
        table.rows = [row for row in table.rows if not (not resolved_where or self._evaluate_where(row, resolved_where))]
